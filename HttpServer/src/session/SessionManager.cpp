...
SessionManager::SessionManager(std::unique_ptr<SessionStorage> storage)
    ...
    , rng_(std::random_device{}()) // 初始化随机数生成器，用于生成随机的会话 ID
{}

// 从请求中获取或创建会话，如果请求中包含会话 ID，则从存储中加载会话，否则创建一个新的会话
...
{   
    ...
    session->refresh();
    storage_->save(session);  // 这里可能有问题，保存会话
    return session;
}
